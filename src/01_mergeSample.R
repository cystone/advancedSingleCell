...
dir.create('./output/01_mergeSample/cluster2', recursive=T)
dir.create('./output/01_mergeSample/cluster3', recursive=T)
...
#计算主成分
{
  ...
  plotc <- plot1+plot2
  ggsave("./output/01_mergeSample/cluster1/pca.png", plot = plotc, width = 8, 
         height = 4)
...
# 降维和非线性聚类
{
  ...
  write.csv(cell_cluster,'./output/01_mergeSample/cluster1/cell_cluster.csv',
            row.names = F)
  ...
  ggsave("./output/01_mergeSample/cluster1/tSNE.png", plot = plot1, width = 8,
         height = 7)
  #group_by_sample
  plot2 = DimPlot(scRNA1, reduction = "tsne", group.by='orig.ident') 
  ggsave("./output/01_mergeSample/cluster1/tSNE_sample.png", plot = plot2, 
         width = 8, height = 7)
  #combinate
  plotc <- plot1+plot2
  ggsave("./output/01_mergeSample/cluster1/tSNE_cluster_sample.png", plot = plotc, 
         width = 10, height = 5)
  ...
  ggsave("./output/01_mergeSample/cluster1/UMAP.png", plot = plot3, width = 8, 
         height = 7)
  #group_by_sample
  plot4 = DimPlot(scRNA1, reduction = "umap", group.by='orig.ident')
  ggsave("./output/01_mergeSample/cluster1/UMAP.png", plot = plot4, width = 8, 
         height = 7)
  #combinate
  plotc <- plot3+plot4
  ggsave("./output/01_mergeSample/cluster1/UMAP_cluster_sample.png", plot = plotc,
         width = 10, height = 5)
}
...
ggsave("./output/01_mergeSample/cluster1/tSNE_UMAP.png", plot = plotc, 
       width = 10, height = 5)
...
ggsave("./output/01_mergeSample/QC/vlnplot_before_qc.pdf", plot = violin,
       width = 12, height = 6) 
ggsave("./output/01_mergeSample/QC/vlnplot_before_qc.png", plot = violin, 
       width = 12, height = 6)  
plot1 <- FeatureScatter(scRNA, feature1 = "nCount_RNA", feature2 = "percent.mt")
plot2 <- FeatureScatter(scRNA, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
plot3 <- FeatureScatter(scRNA, feature1 = "nCount_RNA", feature2 = "percent.HB")
pearplot <- CombinePlots(plots = list(plot1, plot2, plot3), nrow=1, legend="none") 
ggsave("./output/01_mergeSample/QC/pearplot_before_qc.pdf", plot = pearplot, 
       width = 12, height = 5) 
ggsave("./output/01_mergeSample/QC/pearplot_before_qc.png", plot = pearplot,
       width = 12, height = 5)
...
violin <-VlnPlot(scRNA, group.by = "proj_name",
  ...
  theme(axis.title.x=element_blank(), axis.text.x=element_blank(), 
        axis.ticks.x=element_blank()) 
ggsave("./output/01_mergeSample/QC/vlnplot_after_qc.pdf", plot = violin, 
       width = 12, height = 6) 
ggsave("./output/01_mergeSample/QC/vlnplot_after_qc.png", plot = violin, 
       width = 12, height = 6)
...
#计算主成分
{
  ...
  plotc <- plot1+plot2
  ggsave("./output/01_mergeSample/cluster3/pca.png", plot = plotc, width = 8, 
         height = 4)
  print(c("请选择哪些pc轴用于后续分析？示例如下：","pc.num=1:15"))
}
...
# 降维和非线性聚类
{
  ...
  cell_cluster <- data.frame(cell_ID=rownames(metadata), 
                             cluster_ID=metadata$seurat_clusters)
  write.csv(cell_cluster,'./output/01_mergeSample/cluster3/cell_cluster.csv',
            row.names = F)
  
  ##非线性降维
  #tSNE
  scRNA = RunTSNE(scRNA, dims = pc.num)
  embed_tsne <- Embeddings(scRNA, 'tsne')   #提取tsne图坐标
  write.csv(embed_tsne,'./output/01_mergeSample/cluster3/embed_tsne.csv')
  #group_by_cluster
  plot1 = DimPlot(scRNA, reduction = "tsne", label=T) 
  ggsave("./output/01_mergeSample/cluster3/tSNE.png", plot = plot1, width = 8,
         height = 7)
  #group_by_sample
  plot2 = DimPlot(scRNA, reduction = "tsne", group.by='orig.ident') 
  ggsave("./output/01_mergeSample/cluster3/tSNE_sample.png", plot = plot2, 
         width = 8, height = 7)
  #combinate
  plotc <- plot1+plot2
  ggsave("./output/01_mergeSample/cluster3/tSNE_cluster_sample.png", plot = plotc, 
         width = 10, height = 5)
  
  #UMAP
  scRNA <- RunUMAP(scRNA, dims = pc.num)
  embed_umap <- Embeddings(scRNA1, 'umap')   #提取umap图坐标
  write.csv(embed_umap,'./output/01_mergeSample/cluster3/embed_umap.csv') 
  #group_by_cluster
  plot3 = DimPlot(scRNA, reduction = "umap", label=T) 
  ggsave("./output/01_mergeSample/cluster3/UMAP.png", plot = plot3, width = 8, 
         height = 7)
  #group_by_sample
  plot4 = DimPlot(scRNA, reduction = "umap", group.by='orig.ident')
  ggsave("./output/01_mergeSample/cluster3/UMAP.png", plot = plot4, width = 8, 
         height = 7)
  #combinate
  plotc <- plot3+plot4
  ggsave("./output/01_mergeSample/cluster3/UMAP_cluster_sample.png", plot = plotc,
         width = 10, height = 5)
}
...
#合并tSNE与UMAP
plotc <- plot2+plot4+ plot_layout(guides = 'collect')
ggsave("./output/01_mergeSample/cluster3/tSNE_UMAP.png", plot = plotc, 
       width = 10, height = 5)


# refdata <- MonacoImmuneData()
# save(refdata, file = '~/database/SingleR_ref/ref_MonacoImmuneData.RData')
load(file = '~/database/SingleR_ref/ref_MonacoImmuneData.RData')
...
write.csv(celltype,"./output/01_mergeSample/CellType/celltype_Monaco.csv",
          row.names = F)
scRNA@meta.data$celltype_Monaco = "NA"
for(i in 1:nrow(celltype)){
  ind = which(scRNA@meta.data$seurat_clusters == celltype$ClusterID[i])
  scRNA@meta.data[ind,'celltype_Monaco'] <- celltype$celltype[i]}
p1 = DimPlot(scRNA, group.by="celltype_Monaco", repel=T, label=T, label.size=5,
             reduction='tsne')
p2 = DimPlot(scRNA, group.by="celltype_Monaco", repel=T, label=T, label.size=5,
             reduction='umap')
p3 = p1+p2+ plot_layout(guides = 'collect')
ggsave("./output/01_mergeSample/CellType/tSNE_celltype_Monaco.png", p1, 
       width=7 ,height=6)
ggsave("./output/01_mergeSample/CellType/UMAP_celltype_Monaco.png", p2, 
       width=7 ,height=6)
ggsave("./output/01_mergeSample/CellType/celltype_Monaco.png", p3, width=10 ,
       height=5)
#使用DICE参考数据库鉴定
# refdata <- DatabaseImmuneCellExpressionData()
# save(refdata, file='~/database/SingleR_ref/ref_DICE_1561s.RData')
...
celltype = data.frame(ClusterID=rownames(cellpred), celltype=cellpred$labels, 
                      stringsAsFactors = F)
write.csv(celltype,"./output/01_mergeSample/CellType/celltype_DICE.csv",
          row.names = F)
scRNA@meta.data$celltype_DICE = "NA"
for(i in 1:nrow(celltype)){
  ind = which(scRNA@meta.data$seurat_clusters == celltype$ClusterID[i])
  scRNA@meta.data[ind,'celltype_DICE'] <- celltype$celltype[i]}
p4 = DimPlot(scRNA, group.by="celltype_DICE", repel=T, label=T, label.size=5, 
             reduction='tsne')
p5 = DimPlot(scRNA, group.by="celltype_DICE", repel=T, label=T, label.size=5, 
             reduction='umap')
p6 = p3+p4+ plot_layout(guides = 'collect')
ggsave("./output/01_mergeSample/CellType/tSNE_celltype_DICE.png", p4, width=7,
       height=6)
ggsave("./output/01_mergeSample/CellType/UMAP_celltype_DICE.png", p5, width=7,
       height=6)
ggsave("./output/01_mergeSample/CellType/celltype_DICE.png", p6, width=10,
       height=5)
#对比两种数据库鉴定的结果
p8 = p1+p4
